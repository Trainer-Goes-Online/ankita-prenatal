...
export default function CheckoutForm() {
  ...
  async function handlePaymentSuccess(response: RazorpayResponse, dialCode: string) {
    try {
      ...
      // The conversion itself fires server-side as the custom 'sales' event via
      // CAPI - no browser-side Purchase. Under the Health & Wellness data-source
      // restriction the standard 'Purchase' event is restricted by name, so
      // firing it browser-side adds no value and leaks a health-y content_name.
      await setMetaAdvancedMatching({
        email: fields.email,
